#!/usr/bin/env python
### function similar as prepare_craco but just used to perform calibration
import os
import re
import sys
import glob
...
def _make_scan_run(obssbid, scan, ts, runname="cal", target="field"):
    _obssbid = _format_sbid(obssbid)
    _obssbid_s = _format_sbid(obssbid, padding=False)
...
"""
    ...
    _bashcmd = f"""{_bashcmd}
{cfg.MPIPIPESH_PATH} --cardcap-dir $indir --outdir $outdir """


    _bashcmd += f"""--fcm $fcm --metadata $meta --xclbin $XCLBIN --pol-sum --block $block --card $card """
    _bashcmd += f"""--max-ncards {cfg.MAX_NCARDS} --ncards-per-host {cfg.NCARDS_PER_HOST} -N {cfg.N} """
    ...
    if target == "field":
        _bashcmd += f"""--save-uvfits-beams 0-35 """
    else:
        _bashcmd += f"""--save-uvfits-beams {int(scan)} """
    _bashcmd += f"""2>&1 | tee {runoutpath}/piperun.{scan}.{ts}.cal.out\n"""
    
    runshfname = f"{runscriptpath}/runpipe.{scan}.{ts}.cal.sh"
...
def make_run(obssbid, runname="cal", target="field"):
    """
    ...
    for scan, ts in allscans:
        ...
        _make_scan_run(obssbid, scan, ts, runname=runname, target=target)
...
def main():
    ...
    parser.add_argument("-r", "--run", type=str, help="run name, `results` by default", default="cal")
    parser.add_argument("-t", "--target", type=str, help="target of the field", default="field")

    values = parser.parse_args()

    run_prepare(values.obs,)
    make_run(values.obs, values.run, values.target)
